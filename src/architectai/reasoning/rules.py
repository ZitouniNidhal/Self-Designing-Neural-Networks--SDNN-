...
class NoCyclesRule(Rule):
    def __init__(self):
        super().__init__("no_cycles", "Ensures architecture is a Directed Acyclic Graph")
...
class HasInputRule(Rule):
    def __init__(self):
        super().__init__("has_input", "Ensures graph has at least one input node")
...
class HasOutputRule(Rule):
    def __init__(self):
        super().__init__("has_output", "Ensures graph has at least one linear/output node")

    def apply(self, graph: ArchitectureGraph) -> Tuple[bool, str]:
        outputs = [n for n in graph.primitives.values() if n.config.op == OperationType.LINEAR]
...
class MaxDepthRule(Rule):
    def __init__(self, max_depth: int = 100):
        super().__init__("max_depth", f"Ensures graph depth does not exceed {max_depth}")
