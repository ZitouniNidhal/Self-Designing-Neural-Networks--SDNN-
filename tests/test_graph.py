...
def test_graph_add_and_validate():
    ...
    input_node = Primitive(
        id="input_1", config=NodeConfig(op=OperationType.IDENTITY), output_shape=[3, 32, 32]
    )
...
def test_graph_connection():
    ...
    node_b = Primitive(
        id="node_b", config=NodeConfig(op=OperationType.LINEAR, params={"out_features": 10})
    )
